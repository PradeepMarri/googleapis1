...
func Grafeas_listoccurrences2Handler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/%s/occurrences%s", cfg.BaseURL, parent_1, queryString)
...
func CreateGrafeas_listoccurrences2Tool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_parent_1_occurrences",
		mcp.WithDescription("Lists occurrences for the specified project."),
		mcp.WithString("parent_1", mcp.Required(), mcp.Description("The name of the project to list occurrences for in the form of\n`projects/[PROJECT_ID]`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Grafeas_listoccurrences2Handler(cfg),
	}
}
