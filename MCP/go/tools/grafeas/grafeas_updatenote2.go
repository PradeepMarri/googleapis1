...
func Grafeas_updatenote2Handler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		name_3Val, ok := args["name_3"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: name_3"), nil
		}
		name_3, ok := name_3Val.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: name_3"), nil
		}
		...
		url := fmt.Sprintf("%s/v1/%s%s", cfg.BaseURL, name_3, queryString)
...
func CreateGrafeas_updatenote2Tool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v1_name_3",
		mcp.WithDescription("Updates the specified note."),
		mcp.WithString("name_3", mcp.Required(), mcp.Description("The name of the note in the form of\n`projects/[PROVIDER_ID]/notes/[NOTE_ID]`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Grafeas_updatenote2Handler(cfg),
	}
}
