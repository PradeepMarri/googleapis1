...
func Grafeas_updatenoteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		name_2Val, ok := args["name_2"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: name_2"), nil
		}
		name_2, ok := name_2Val.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: name_2"), nil
		}
		...
		url := fmt.Sprintf("%s/v1/%s%s", cfg.BaseURL, name_2, queryString)
...
func CreateGrafeas_updatenoteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v1_name_2",
		mcp.WithDescription("Updates the specified note."),
		mcp.WithString("name_2", mcp.Required(), mcp.Description("The name of the note in the form of\n`projects/[PROVIDER_ID]/notes/[NOTE_ID]`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Grafeas_updatenoteHandler(cfg),
	}
}
