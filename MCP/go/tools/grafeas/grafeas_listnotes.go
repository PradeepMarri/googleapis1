...
func Grafeas_listnotesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGrafeas_listnotesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_parent_notes",
		mcp.WithDescription("Lists notes for the specified project."),
		mcp.WithString("parent", mcp.Required(), mcp.Description("The name of the project to list notes for in the form of\n`projects/[PROJECT_ID]`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Grafeas_listnotesHandler(cfg),
	}
}
