...
func Grafeas_listnoteoccurrencesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/%s/occurrences%s", cfg.BaseURL, name, queryString)
...
func CreateGrafeas_listnoteoccurrencesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_name_occurrences",
		mcp.WithDescription("Lists occurrences referencing the specified note. Provider projects can use
this method to get all occurrences across consumer projects referencing the
specified note."),
		mcp.WithString("name", mcp.Required(), mcp.Description("The name of the note to list occurrences for in the form of\n`projects/[PROVIDER_ID]/notes/[NOTE_ID]`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Grafeas_listnoteoccurrencesHandler(cfg),
	}
}
