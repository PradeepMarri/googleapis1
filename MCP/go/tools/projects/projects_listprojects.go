...
func Projects_listprojectsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/projects%s", cfg.BaseURL, queryString)
		...
		var result models.ProjectListProjectsResponse
...
func CreateProjects_listprojectsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_projects",
		mcp.WithDescription("Lists projects."),
		mcp.WithString("filter", mcp.Description("The filter expression.")),
		mcp.WithNumber("pageSize", mcp.Description("Number of projects to return in the list.")),
		mcp.WithString("pageToken", mcp.Description("Token to provide to skip to a particular spot in the list.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Projects_listprojectsHandler(cfg),
	}
}
